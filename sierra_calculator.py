...
class Calculator:
    ...
    def calculate_values_vix(self, rolling_period):
        ...
        self.data_frame[self.column_id + '_Avg'] = self.data_frame['VIX_Last'].rolling(rolling_period).mean()
        self.data_frame[self.column_id + '_Std'] = self.data_frame['VIX_Last'].rolling(rolling_period).std()
